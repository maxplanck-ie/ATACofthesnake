...
def gp_row_test(row):
    with warnings.catch_warnings():
        ...
        LR_obs, p_value, y_pred, y_std = fit_peak_gp(
            ...
            time_grid,
            cov_encoded,
            snakemake.params.permutations,
        )

    return pd.Series(
        {"LR": LR_obs, "pvalue": p_value, "y_pred": y_pred, "y_std": y_std}
    )
...
results_df = pd.DataFrame(
    {"LR": LRs, "pvalue": pvals, "y_pred": ypreds, "y_std": ystds}, index=peaks
)

reject, results_df["FDR"], _, _ = multipletests(results_df["pvalue"], method="fdr_bh")
patterns = np.vstack(
    results_df.loc[results_df["FDR"] < snakemake.params.permutation_cutoff, "y_pred"]
)
...
for k in range(K):
    ...
    ax[k].plot(
        time_grid, patterns_scaled[labels == k].mean(axis=0), color="black", linewidth=2
    )
