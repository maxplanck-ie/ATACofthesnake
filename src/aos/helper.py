...
def plotixs(ixs, mitostring):
    pal = sns.cubehelix_palette(10, rot=-0.25, light=0.7)
    # ixs
    df = pd.read_csv(ixs, sep="\t", header=0, index_col=0, comment="#")
    df = pd.DataFrame(round(df.loc[mitostring] / df.sum(), 2))
    df["sample"] = df.index
    df.sort_values(by=[0], inplace=True, ascending=False)
    g = sns.barplot(data=df, x="sample", y=0, color=pal[5])
    g.tick_params(axis="x", labelrotation=90)
...
def plotsieve(sieve):
    pal = sns.cubehelix_palette(10, rot=-0.25, light=0.7)
    df = pd.read_csv(sieve, sep="\t", header=0, index_col=0, comment="#").T
    df.sort_values(by=["fraction"], inplace=True, ascending=False)
    g = sns.barplot(data=df, x=df.index, y="fraction", color=pal[5])
    g.tick_params(axis="x", labelrotation=90)
    g.set(xlabel="", ylabel="Fraction of surviving reads")
    g.figure.savefig("figures/alignmentsieve.png", dpi=300, bbox_inches="tight")


def maplot(tsv: str, of: str, compentry: dict, lfc_cutoff: float, fdr_cutoff: float):
    ...
    sig_mask = (df["FDR"] < fdr_cutoff) & (np.abs(df["logFC"]) > lfc_cutoff)
    df["sig"] = sig_mask.map({True: "S", False: "NS"})
    upc = int((sig_mask & (df["logFC"] > lfc_cutoff)).sum())
    downc = int((sig_mask & (df["logFC"] <= -lfc_cutoff)).sum())
    g = sns.scatterplot(
        data=df,
        x="logCPM",
        y="logFC",
        ...
        with open(peaks) as f:
            for line in f:
                ...
                if end > chromdic[chrom]:
                    bedlis.append([chrom, start, chromdic[chrom]])
                    peakchange += 1
                else:
                    bedlis.append([chrom, start, end])
        print("Changed {} peaks.".format(peakchange))
        beddf = pd.DataFrame(bedlis)
        beddf.to_csv(of, sep="\t", header=False, index=False)
    else:
...
def PCA_colors(samplesheet, samples):
    ...
    if samplesheet:
        sdf = pd.read_csv(samplesheet, sep="\t", header=0)
