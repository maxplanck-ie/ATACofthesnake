...
class Preflight:
    ...
    def __init__(self, clickdict: dict) -> None:
        ...
        assert self.workflowfile.exists(), (
            f"{self.workflowfile} does not exist next to {__file__}. Please check your installation."
        )
        ...
        self.cutoffs = {
            "permutation_cutoff": clickdict["permutation_cutoff"],
        ...
        # Parse fasta file
        self.logger.info(
            "Validating fasta file and calculating effective genome size..."
        )
        self.parse_fasta()
    ...
    def validate_mitostring(fna, mitostring, rarfile):
        ...
        assert any([mitostring in header for header in headers]), (
            f"Provided mitostring {mitostring} not found in fasta file. Please check your fasta file and the --mitostring parameter. Exiting."
        )
    ...
    def validate_samplesheet(ss: Path, bamdir: Path) -> None:
        ...
        for sample in df["sample"]:
            ...
            assert bamfile.exists() or cramfile.exists(), (
                f"Bamfile or cramfile not found for sample {sample} in samplesheet. Exiting."
            )
    ...
    def validate_comparison(comp_path: Path, ss: Path) -> None:
        ...
        for _ in comps:
            ...
            # Type is present
            assert "type" in comp, (
                f"Comparison {comp} does not have `type` key. Exiting."
            )
            assert comp["type"] in ["twogroup", "lrt", "timecourse"], (
                f"Comparison {comp} has invalid type {comp['type']}. Type should be one of 'twogroup', 'lrt' or 'timecourse'. Exiting."
            )
            ...
            if comp["type"] == "twogroup":
                ...
                _.pop("type", None)
                assert len(_.keys()) == 2, (
                    f"Comparison {comp} of type 'twogroup' should only have two groups specified in the comparison file. Exiting."
                )
                for group in _:
                    for factor in _[group]:
                        ...
                        if isinstance(_[group][factor], str):
                            ...
                            for level in _[group][factor]:
                                assert level in samplesheet[factor].unique(), (
                                    f"Comparison {comp} has level {level} for factor {factor} that is not present in samplesheet. Exiting."
                                )
            if comp["type"] == "lrt":
                assert "reduced" in comp, (
                    f"Comparison {comp} of type 'lrt' should have a 'reduced' key specifying the reduced model. Exiting."
                )
            ...
            if comp["type"] == "timecourse":
                ...
                assert "time_type" in comp, (
                    f"Comparison {comp} of type 'timecourse' should have a 'time_type' key specifying the time variable type (continuous or ordinal). Exiting."
                )
                assert comp["time_type"] in ["continuous", "ordinal"], (
                    f"Comparison {comp} has invalid time_type {comp['time_type']}. Should be 'continuous' or 'ordinal'. Exiting."
                )
                assert comp["time"] in covariates, (
                    f"Comparison {comp} has time variable {comp['time']} that is not present in samplesheet. Exiting."
                )
    ...
    def parse_fasta(self):
        ...
        with open(self.fna, "r") as f:
            for line in f:
                if line.startswith(">"):
                    assert "|" not in line, (
                        f" '|' are not allowed in fasta headers. Please reformat. Offending header: {line.strip()}"
                    )
                else:
    ...
    def parse_gtf(self):
        gtf = pd.read_csv(
            self.gtf, sep="\t", comment="#", header=None, dtype={0: "str"}
        )
        ...
        assert not gtf.empty, (
            f"Feature {self.uro[0]} not found in GTF file. Please check your GTF file and the --featureuro parameter. Exiting."
        )

        gtf = gtf.sort_values(["chr", "start"], ascending=(True, True))
        gtf.to_csv(
            self.outputdir / "genes.sorted.gtf", header=False, index=False, sep="\t"
        )
